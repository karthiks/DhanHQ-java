...
public class LiveQuoteMax {
  ...
  /** Highest Open Interest for the da (only for NSE_FNO) */
  private int oi_highest;

  /** Lowest Open Interest for the day (only for NSE_FNO) */
  private int oi_lowest;
  ...
  public LiveQuoteMax(ByteBuffer buffer) {
    // Extracting values based on format param to struct.unpack(format, bytestream) in python SDK -
    // marketfeed.process_quote()
    /* Warning: Buffer reading follows order sequence as in '<BHBIfHIfIIIIIIffff100s'. Do not change it below: */
    ...
    lastTradeTime =
        Instant.ofEpochSecond(buffer.getInt()); // DateUtils.convertToDateTime(buffer.getInt());
    averagePrice = toBigDecimal(buffer.getFloat());
    volume = buffer.getInt();
    sellQuantity = buffer.getInt();
    buyQuantity = buffer.getInt();
    oi = buffer.getInt();
    oi_highest = buffer.getInt();
    oi_lowest = buffer.getInt();
