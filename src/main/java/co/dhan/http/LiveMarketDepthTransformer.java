...
public class LiveMarketDepthTransformer extends WebSocketListener {
  ...
  // Message Length (320b depth + 12b header) as per API Doc:
  // https://dhanhq.co/docs/v2/20-market-depth/
  ...
  public void onClosed(WebSocket webSocket, int code, String reason) {
    ...
    depthListener.onTermination(
        new DhanAPIException(String.valueOf(code), "Client terminated the connection. " + reason));
  }
  ...
  public void onFailure(WebSocket webSocket, Throwable t, Response response) {
    if (response != null) {
      try (ResponseBody body = response.body()) {
        DhanAPIException e =
            new DhanAPIException(String.valueOf(response.code()), response.body().string());
  ...
  void processMessage(byte[] byteArray) {
    ...
    switch (feedResponseCode) { // FeedResponseCode.findByCode(41) == ??
      ...
      default -> {
        depthListener.onError(
            new Exception("Unknown packet received with feedResponseCode " + byteResponseCode));
      }
    }
  }
  ...
  static byte[][] splitByteArray(byte[] originalArray, int chunkSize) {
    ...
    //        int numOfSubArrays = (totalLength + chunkSize - 1) / chunkSize; // To return sub-array
