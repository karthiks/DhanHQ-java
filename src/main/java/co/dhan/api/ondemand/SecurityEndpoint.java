...
public class SecurityEndpoint {
  ...
  public EDISStatus getEDISStatusOf(String isin) throws DhanAPIException {
    ...
    return dhanConnection.getDhanHTTP().doHttpGetRequest(endpoint).convertToType(EDISStatus.class);
  }
  ...
  public void openBrowserForTPin(
      String isin, int quantity, Exchange exchange, Segment segment, boolean bulk)
    ...
    DhanResponse dhanResponse =
        dhanConnection.getDhanHTTP().doHttpPostRequest(APIEndopint.EdisForm, payload);
  ...
  private File createTempHtmlFile(String formHtml) throws IOException {
    ...
    File tempFile =
        File.createTempFile(APIEndopint.TempWebFormPrefix, APIEndopint.TempWebFormSufix);
  ...
  private void openInBrowser(File tempFile) throws IOException {
    if (Desktop.isDesktopSupported()) {
      ...
      String msg =
          String.format(
              "Desktop is not supported. Cannot open browser automatically. "
