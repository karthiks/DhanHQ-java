...
public class StatementEndpoint {
  ...
  public Ledger getLedgerReport(LocalDate fromDate, LocalDate toDate) throws DhanAPIException {
    ...
    String endpoint =
        String.format(APIEndpoint.LedgerReport, fromDate.format(pattern), toDate.format(pattern));
    return dhanConnection.getDhanHTTP().doHttpGetRequest(endpoint).convertToType(Ledger.class);
  }
}
