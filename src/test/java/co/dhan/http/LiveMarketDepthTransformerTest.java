...
class LiveMarketDepthTransformerTest extends UnitTestRoot {
  ...
  private byte[] liveDepthToByteArray(LiveDepth ld) {
    // headerByteSize = 2b(messageLength=332) + 1b(FeedResponseCode) + 1b(ExchangeSegmentCode) +
    // 4b(SecurityID) + 4b(IgnoreMsg)
    ...
    byte[] twenty_depth_bids_packet =
        buildBidsPacketFrom(
            ld,
            messageLength,
            twentyDepthBidsPacketCode,
            exchangeSegmentCode,
            securityID,
            randomNumberToIgnore);

    byte[] twenty_depth_asks_packet =
        buildAsksPacketFrom(
            ld,
            messageLength,
            twentyDepthAsksPacketCode,
            exchangeSegmentCode,
            securityID,
            randomNumberToIgnore);
  ...
  private byte[] buildBidsPacketFrom(
      LiveDepth ld,
      short messageLength,
    ...
    ByteBuffer bidsBuffer = ByteBuffer.allocate(messageLength).order(ByteOrder.LITTLE_ENDIAN);
  ...
  private byte[] buildAsksPacketFrom(
      LiveDepth ld,
      short messageLength,
      byte feedResponseCode,
      byte exchangeSegmentCode,
