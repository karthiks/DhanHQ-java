...
class PortfolioEndpointTest extends UnitTestRoot {
  ...
  @Spy @InjectMocks PortfolioEndpoint portfolioEndpoint;
  ...
  void getCurrentHoldings_ReturnListOfHoldings() {
    ...
    when(mockDhanResponse.convertToType((TypeReference<List<Holding>>) any()))
  ...
  void getCurrentPositions_ReturnListOfHoldings() {
    ...
    when(mockDhanConnection.getDhanHTTP()).thenReturn(mockDhanHTTP);
    when(mockDhanHTTP.doHttpGetRequest(anyString())).thenReturn(mockDhanResponse);
    when(mockDhanResponse.convertToType((TypeReference<List<Position>>) any()))
  ...
  void convertPosition() {
    ...
    expectedPayload.put(
        PortfolioEndpoint.APIParam.ExchangeSegment,
        currentPosition.getExchangeSegment().toString());
    expectedPayload.put(
        PortfolioEndpoint.APIParam.PositionType, currentPosition.getPositionType().toString());
    expectedPayload.put(
        PortfolioEndpoint.APIParam.FromProductType, currentPosition.getProductType().toString());
