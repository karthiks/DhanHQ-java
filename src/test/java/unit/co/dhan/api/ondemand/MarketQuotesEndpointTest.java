...
public class MarketQuotesEndpointTest extends UnitTestRoot {
  ...
  @InjectMocks private MarketQuotesEndpoint marketQuotesEndpoint;

  @Test
  public void getLTPFor_ShouldReturnResult()
      throws DhanAPIException, IOException, URISyntaxException {
    ...
    when(mockDhanHTTP.doHttpPostRequest(
            GetLTPForSecurities, exchangeSegmentSecurities.toMapOfStrings()))
        .thenReturn(stubDhanResponse);

    Map<ExchangeSegment, List<Candlestick>> map =
        marketQuotesEndpoint
    ...
    assertThat(map.get(ExchangeSegment.NSE_FNO))
        .isEqualTo(List.of(new Candlestick("49081", "368.15"), new Candlestick("49082", "694.35")));
  }

  @Test
  public void getOHLCFor_ShouldReturnResult()
      throws DhanAPIException, URISyntaxException, IOException {
    ...
    when(mockDhanConnection.getDhanHTTP()).thenReturn(mockDhanHTTP);
    when(mockDhanHTTP.doHttpPostRequest(
            GetOHLCForSecurities, exchangeSegmentSecurities.toMapOfStrings()))
        .thenReturn(stubDhanResponse);
    ...
    assertThat(map.get(ExchangeSegment.NSE_EQ))
        .isEqualTo(
            List.of(new Candlestick("11536", "4525.55", "4521.45", "4507.85", "4530", "4500")));
    assertThat(map.get(ExchangeSegment.NSE_FNO))
        .isEqualTo(
            List.of(
  ...
  @Test
  public void getQuoteFor_ShouldReturnResult()
      throws DhanAPIException, IOException, URISyntaxException {
    ...
    when(mockDhanConnection.getDhanHTTP()).thenReturn(mockDhanHTTP);
    when(mockDhanHTTP.doHttpPostRequest(eq(GetQuoteForSecurities), anyMap()))
        .thenReturn(stubDhanResponse);
    Map<ExchangeSegment, List<Quote>> map =
        marketQuotesEndpoint.getQuoteFor(exchangeSegmentSecurities).getExchangeSegmentQuotesMap();
