...
class LiveMarketDepthTest {
    ...
    void command_throwsDhanAPIException_whenInstrumentsListIsNull() {
        assertThatThrownBy(() -> liveMarketDepth.command(null, FeedRequestCode.SUBSCRIBE_20_LEVEL_DEPTH))
            .isInstanceOf(DhanAPIException.class)
            .hasMessage("Instruments list cannot be empty and cannot be greater than 100 in one batch.");
    }

    @Test
    void command_throwsDhanAPIException_whenInstrumentsListIsEmpty() {
        List<Instrument> emptyList = new ArrayList<>();
        assertThatThrownBy(() -> liveMarketDepth.command(emptyList, FeedRequestCode.SUBSCRIBE_20_LEVEL_DEPTH))
            .isInstanceOf(DhanAPIException.class)
            .hasMessage("Instruments list cannot be empty and cannot be greater than 100 in one batch.");
    }
    ...
    void command_throwsDhanAPIException_whenInstrumentsListExceeds100() {
        ...
        assertThatThrownBy(() -> liveMarketDepth.command(largeList, FeedRequestCode.SUBSCRIBE_20_LEVEL_DEPTH))
            .isInstanceOf(DhanAPIException.class)
            .hasMessage("Instruments list cannot be empty and cannot be greater than 100 in one batch.");
    }
