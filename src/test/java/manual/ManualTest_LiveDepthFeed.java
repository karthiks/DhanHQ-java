...
public class ManualTest_LiveDepthFeed {
  //    private static final String SERVER_URL = "wss://stockmarket.example.com/ws"; // Replace with
  // your server URL
  ...
  public static void main(String[] args) {
    ...
    LiveMarketDepthListener liveMarketDepthListener =
        new LiveMarketDepthListener() {
          public void onConnection() {
            log.debug("LiveMarketDepthListener.onConnection():");
            //                Instrument iNTPCGreen = new Instrument(ExchangeSegment.NSE_EQ,
            // "27176");
            ...
            liveMarketDepth.command(List.of(instrument), FeedRequestCode.SUBSCRIBE_20_LEVEL_DEPTH);
          }
          ...
          public void onBidsArrival(LiveDepth liveDepth) {
            log.debug(
                "LiveMarketDepthListener.onBidsArrival(..), size "
                    + liveDepth.getBuy().size()
                    + " for securityID "
                    + liveDepth.getSecurityID());
          ...
          public void onAsksArrival(LiveDepth liveDepth) {
            log.debug(
                "LiveMarketDepthListener.onAsksArrival(..), size "
                    + liveDepth.getSell().size()
                    + " for securityID "
                    + liveDepth.getSecurityID());
