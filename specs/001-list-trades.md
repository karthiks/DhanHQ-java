# Spec: Implementation Spec: GET /trades - Retrieve List of All Trades for the Day
...
**Model Requirements**  
Create a new POJO model class named `Trade` if it does not exist. It must include all the following fields with proper Jackson annotations (`@JsonProperty`), getters, setters, and a no-arg constructor (follow the exact style of existing models like `Order` or `Position` in the project):
```java
- String dhanClientId
- String orderId
